...
RUN apt-get update && \
  apt-get install procps python3 python3-pip python3-dev gdb git make file gcc wget binutils -y
...
RUN bash -c "$(wget -q https://github.com/hugsy/gef/raw/master/scripts/gef.sh -O -)"
RUN bash -c "$(wget -q https://github.com/hugsy/gef/raw/master/scripts/gef-extras.sh -O -)"
